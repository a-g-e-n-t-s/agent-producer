...
import {BaseBot, BaseBotConfig, logger, MODULE_DISCORD_BOT, timer, readConfig} from 'agents-library';
...
export class DiscordBot extends BaseBot {
    ...
    try {
      ...
      logger.debug(MODULE_DISCORD_BOT, `Tools being sent to LLM (${openaiTools.length} total)`, timer.elapsed('main'));
      ...
      while (iteration < maxIterations && !finalResponse) {
        ...
        logger.debug(MODULE_DISCORD_BOT, `=== Iteration ${iteration} ===`, timer.elapsed('main'));
        logger.debug(MODULE_DISCORD_BOT, `Sending ${messages.length} messages to LLM with model: ${detectedModel || 'default'}${toolsExecuted ? ' (tools disabled - already executed)' : ''}`, timer.elapsed('main'));
        ...
        logger.debug(MODULE_DISCORD_BOT, `Message roles: [${msgSummary}]`, timer.elapsed('main'));
        ...
        if (hasTools) {
          ...
          logger.debug(MODULE_DISCORD_BOT, `Using NON-STREAMING mode (tools present)`, timer.elapsed('main'));
          ...
          logger.debug(MODULE_DISCORD_BOT, `Non-streaming response complete (${botResponse.length} chars)`, timer.elapsed('main'));
        } else {
          // Streaming mode for final text responses
          logger.debug(MODULE_DISCORD_BOT, `Using STREAMING mode (no tools)`, timer.elapsed('main'));
          ...
          try {
            ...
            logger.debug(MODULE_DISCORD_BOT, `Streamed response complete (${botResponse.length} chars)`, timer.elapsed('main'));
        ...
        if (toolCallData && toolCallData.toolCalls.length > 0) {
          ...
          logger.debug(MODULE_DISCORD_BOT, `Messages array now has ${messages.length} messages, continuing to iteration ${iteration + 1}`, timer.elapsed('main'));
        ...
        logger.debug(MODULE_DISCORD_BOT, `Received final text response (no tool calls), breaking loop`, timer.elapsed('main'));
        finalResponse = botResponse;
        break;
      }

      logger.debug(MODULE_DISCORD_BOT, `Completed iteration ${iteration}, maxIterations: ${maxIterations}, finalResponse: ${finalResponse ? 'YES' : 'NO'}`, timer.elapsed('main'));
